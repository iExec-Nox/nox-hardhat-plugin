 ...
 *
 *         Balances and transfers rely on the Nox TEE; anything that actually
 *         touches encrypted state (mint/transfer/…) only works when the Nox
 *         stack is running — which is exactly what this Hardhat plugin spins
 *         up via `hardhat test`.
 */
